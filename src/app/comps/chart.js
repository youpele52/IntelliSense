...
export default function Chart({ chartData }) {
  // create DOM reference for chart, chart canvass
  ...
  useEffect(() => {
    if (chartContainer?.current && chartData.length > 0) {
        ...
        data: {
          ...
          datasets: chartData.map((entry, i) => {
            return {
              ...
              borderWidth: 3,
              lineTension: 0.3, //curves
            }
          }),
        },
        options: {
          plugins: { title: { display: true, text: 'TK1 Line Chart' } },
          scales: {
      ...
      // using ref, access the dom element use .current
      const newChartInstance = new Chartjs(chartContainer.current, chartConfig)
      setChartInstance(newChartInstance)
    }
    // when the entire component unmount, the chart instance is destroyed
